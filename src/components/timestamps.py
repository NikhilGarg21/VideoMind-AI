...
class TimestampGenerator:
    ...
    def generate_timestamps(
        ...
        call, that payload is salvaged directly instead of retrying.
        ...
        try:
            ...
            structured_llm = self.llm.with_structured_output(
                {
                    ...
                    "description": (
                        "Semantic topics identified from " "a video transcript"
                    ),
                    "type": "object",
                    "properties": {
                        "topics": {
            ...
            for attempt in range(
                ...
                try:
                    ...
                    logger.warning(
                        f"LLM tool-call attempt " f"{attempt}/{max_retries} failed: {e}"
                    )
    ...
    def validate_topics(
        ...
        Args:
            ...
            segments: The original transcript segments.
        ...
        try:
            ...
            for topic in topics:
                ...
                if start_segment not in valid_segment_ids:
                    raise ValueError(f"Invalid start segment ID: " f"{start_segment}")

                if end_segment not in valid_segment_ids:
                    raise ValueError(f"Invalid end segment ID: " f"{end_segment}")
    ...
    def convert_segments_to_timestamps(
        self,
        topics,
        segments,
    ):
        ...
        Returns:
            A list of topic dicts with `topic_id`, `topic`,
    ...
    def _to_seconds(
        ...
        """
        Convert a `MM:SS` timestamp into total seconds.
        """
    ...
    def close_gaps(
        ...
        """
        Close small timing gaps between consecutive topics.
        """
        try:
    ...
    def save_timestamps(
        ...
        """
        Save the final list of topic timestamps.
        """
        try:
    ...
    def initiate_timestamp_generation(
        self,
    ) -> TimestampArtifact:
        """
        Execute the complete timestamp generation pipeline.
        """
        try:

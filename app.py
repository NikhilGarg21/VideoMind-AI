...
STAGE_DEFS = [
    {"key": "ingestion", "label": "Ingest", "desc": "Pulling the audio track from the source."},
    {"key": "transcription", "label": "Transcribe", "desc": "Listening to the audio, word by word, with exact timing."},
    {"key": "text_processing", "label": "Structure", "desc": "Splitting the transcript into meaning-sized pieces."},
    {"key": "timestamp", "label": "Chapters", "desc": "Finding where the topic actually changes — not just every N minutes."},
    {"key": "summary", "label": "Summary", "desc": "Writing the TL;DR and checking its numbers against the transcript."},
    {"key": "embedding", "label": "Index", "desc": "Making every moment searchable, so you can ask it anything."},
]
...
def public_job_view(job: dict) -> dict:
    ...
    if job.get("results"):
        view["results"] = {**job["results"], "qa_ready": job.get("qa_pipeline") is not None}
    return view
...
def ingest_uploaded_file(job_id: str, saved_path: str, original_filename: str, pipeline: VideoPipeline) -> AudioIngestionArtifact:
    """
    ...
    subprocess.run(
        ["ffmpeg", "-y", "-i", saved_path, "-vn", "-acodec", "libmp3lame", "-q:a", "2", config.audio_path],
    ...
    chunk_durations = audio_ingestion.compute_chunk_durations(duration, config.chunk_duration)
    ...
    video_metadata_file_path = save_json(video_metadata, config.video_metadata_file_path)
...
def run_job(job_id: str, source_type: str, source_value: str, original_filename: Optional[str] = None) -> None:
    ...
    with PIPELINE_LOCK:
        try:
            ...
            if source_type == "url":
                ...
                ingestion_artifact = ingest_uploaded_file(job_id, source_value, original_filename, pipeline)
                video_meta = load_json(ingestion_artifact.video_metadata_file_path)
                with JOBS_LOCK:
                    JOBS[job_id]["media_url"] = f"/media/{job_id}/{os.path.basename(source_value)}"
            ...
            text_processing_artifact = pipeline.start_text_processing(transcription_artifact)
            ...
            segments = load_json(transcription_artifact.transcript_file_path)["segments"]
            transcript = [
                {
...
def ask_question(job_id: str, payload: AskRequest):
    ...
    if qa_pipeline is None:
        raise HTTPException(status_code=409, detail="This video isn't ready for questions yet")
